...
Когда мы указываем метод-обработчик непосредственно:
```
closeImageElement.addEventListener('click', this.close);
```
теряется нужный нам контекст внутри метода обработчика.

Когда мы определяем обработчик, используя стрелочную функцию:
```
closeImageElement.addEventListener('click', () => this.close());
```
Мы сохраняем контекст тот, который был в момент определения обработчика события.
