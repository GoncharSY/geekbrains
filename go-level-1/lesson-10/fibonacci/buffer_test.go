...
func TestCalculateWithBuffer(t *testing.T) {
	var cases = []struct {
		Input  int
		Output int
		IsErr  bool
	...
	for _, cs := range cases {
		val, err := CalculateWithBuffer(cs.Input, nil)
...
func BenchmarkCalculateWithBuffer(b *testing.B) {
	for i := 0; i < b.N; i++ {
		CalculateWithBuffer(20, nil)
	}
}
